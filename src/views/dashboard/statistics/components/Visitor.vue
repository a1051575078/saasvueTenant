<template>
    <div>
        ...
        <el-table :data="tableData" border fit highlight-current-row style="width:100%" size="mini" :max-height="height+'px'">
            <el-table-column prop="created_at" label="创建时间" align="center" min-width="20%" sortable>
                <template slot-scope="scope">
                    <i class="el-icon-time"/>
                    <span style="margin-left:10px">{{scope.row.created_at}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="frequency" label="访问次数" align="center" min-width="10%">
                <template slot-scope="scope">
                    <span style="margin-left: 10px">{{scope.row.frequency}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="isphone" label="设备" align="center" min-width="15%">
                <template slot-scope="scope">
                    <span style="margin-left: 10px">{{scope.row.isphone}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="origin" label="来访渠道" align="center" min-width="20%">
                <template slot-scope="scope">
                    <span style="margin-left: 10px">{{scope.row.origin}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="ip" label="访问IP" align="center" min-width="15%">
                <template slot-scope="scope">
                    <span style="margin-left: 10px">{{scope.row.ip}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="address" label="IP地理位置" align="center" min-width="20%">
                <template slot-scope="scope">
...
export default {
    ...
    mounted() {
        this.allTableData=this.visitordata;
        this.filterTableData=this.visitordata;
        this.fuzzySearch=this.visitordata;
        //设置分页数据
        this.setPaginations();
    },
    created(){
